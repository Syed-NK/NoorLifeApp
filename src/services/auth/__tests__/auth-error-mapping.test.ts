...
describe('configuration faults', () => {
  ...
  it('maps PostgREST PGRST125 to not-configured', () => {
    expect(
      toAuthErrorCode({ code: 'PGRST125', message: 'Invalid path specified in request URL' }),
    ).toBe('not-configured');
  });
  ...
  it('does not report a configuration fault as a server error', () => {
    ...
    expect(
      toAuthErrorCode({ status: 404, message: 'Invalid path specified in request URL' }),
...
describe('authorization faults', () => {
  it('maps Postgres 42501 permission denied to session-expired', () => {
    expect(
      toAuthErrorCode({ code: '42501', message: 'permission denied for table profiles' }),
...
describe('rate limits are told apart', () => {
  ...
  it('classifies the email quota separately from a request rate limit', () => {
    ...
    expect(toAuthErrorCode({ status: 429, message: 'email rate limit exceeded' })).toBe(
      'email-rate-limited',
    );
...
describe('signup and sign-in faults', () => {
  it.each([
    ...
    [
      { status: 400, code: 'email_address_invalid', message: 'Email address "x@y" is invalid' },
      'invalid-email',
    ],
  ...
  it('passes an AuthError through unchanged', () => {
    expect(toAuthErrorCode(new AuthError('provider-not-configured'))).toBe(
