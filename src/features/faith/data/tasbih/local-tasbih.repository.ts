...
/** The stored form of a session, version stamped. */
function toStored(session: TasbihSession): Record<string, unknown> {
  return { version: TASBIH_SCHEMA_VERSION, ...session };
}
...
export function createLocalTasbihRepository(): TasbihRepository {
  ...
  /**
   * Reads the session, migrating a legacy record exactly once.
   *
   * `unreadable` returns `null` rather than rewriting: storage is left exactly as it is, and the
   * caller starts a fresh counter without destroying whatever could not be interpreted.
   */
  async function loadSession(): Promise<TasbihSession | null> {
    ...
    switch (parsed.kind) {
      ...
      case 'migrated':
        // Written once. A failed write is not fatal — the next read migrates again, identically.
        await writeChecked(faithStorageKeys.tasbihSession, toStored(parsed.session));
        return parsed.session;
      default:
        return null;
    }
  ...
  async function mutate(
    ...
    const run = pending.then(async (): Promise<FaithResult<TasbihSession>> => {
      const current =
        (await loadSession()) ?? freshSession(DEFAULT_COUNTER.id, DEFAULT_COUNTER.target);
      const next = change(current);
      const written = await writeChecked(faithStorageKeys.tasbihSession, toStored(next));
      if (!written) {
  ...
  /** Starts a counter, archiving whatever was in progress. Shared by `startSession` and `reset`. */
  async function begin(counterId: string): Promise<FaithResult<TasbihSession>> {
    const labels = await loadLabels();
    const label = labels.find((item) => item.id === counterId) ?? DEFAULT_COUNTER;
    const current = await loadSession();
    if (current !== null) {
      await archive(current);
    }
    const next = freshSession(label.id, label.target);
    const written = await writeChecked(faithStorageKeys.tasbihSession, toStored(next));
    if (!written) {
      return { kind: 'error', code: 'unavailable', detail: 'tasbih write failed' };
    }
  ...
  return {
    ...
    async deleteLabel(id: string): Promise<FaithResult<readonly CounterLabel[]>> {
      ...
      if (session !== null && session.counterId === id) {
        // The counter being deleted is the active one; fall back rather than leave it dangling.
        await begin(DEFAULT_COUNTER.id);
      }
    ...
    async getSession(): Promise<FaithResult<TasbihSession>> {
      ...
      return stored === null ? { kind: 'empty' } : { kind: 'ok', data: stored };
    },

    async startSession(counterId: string): Promise<FaithResult<TasbihSession>> {
      return begin(counterId);
    },
    ...
    async reset(): Promise<FaithResult<TasbihSession>> {
      const current = await loadSession();
      return begin(current?.counterId ?? DEFAULT_COUNTER.id);
    },
