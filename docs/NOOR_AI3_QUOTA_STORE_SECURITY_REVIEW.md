...
| **B10** | **HMAC key lifecycle — provisioning and rotation.** No local/CI provisioning path exists, so `supabase db reset` yields a store that migrates cleanly and fails at first call; and no rotation procedure is specified for any of the three keys (who rotates, on what trigger, verified how). Scope: **getting a key into every environment and changing it safely**, distinct from B3's "who can read it" | **PARTIALLY ADDRESSED 2026-08-09** — the *provider safety-identifier* key now has a written construction, validation, storage policy, access path, rotation procedure and emergency response (`NOOR_AI_B10_SAFETY_IDENTIFIER_RUNBOOK.md`), implemented and locally verified with **no key generated and no secret provisioned**. The quota-store keys and the local/CI provisioning path are **untouched** | A provisioning step + a rotation procedure per key + T-19 |
...
This says nothing about the provider `safety_identifier`, which is a **different value with a different
threat model**: the store's `subject_id` is a raw uuid inside NoorLife's own database, while the safety
identifier is a derived value crossing to a third party.

**Updated 2026-08-09.** That second value is now decided and implemented. Its "secret salt"
requirement stands in a stronger form — a **dedicated random 256-bit HMAC key**, independent of every
other key, read only by `supabase/functions/noor-ai/safety-identifier.ts`, imported non-extractably
with `sign` as its only usage. The reasoning is the same one §21.3 gives above and is why an unkeyed
digest was rejected there too: NoorLife holds the complete uuid list, so an unkeyed digest of a uuid is
reversible by anyone who obtains it. See `NOOR_AI_BACKEND_CONTRACT.md` §12.6 and
`NOOR_AI_B10_SAFETY_IDENTIFIER_RUNBOOK.md`.

**B10 is not closed by this.** What is closed is the safety-identifier key's construction, validation,
storage policy, access path, rotation procedure and emergency response — written and locally verified,
with **no key generated and no secret provisioned**. What remains open is everything else B10 covers:
the quota store's own key provisioning and rotation, the local/CI provisioning path for any key, and
hosted verification of any of it.
